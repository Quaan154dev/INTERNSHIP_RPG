...
export default function Scene() {
  ...
  const [storyText, setStoryText] = useState('Phương là sinh viên chuyên ngành Nodejs...');
  ...
  const handleNext = () => {
    switch (step) {
      case 0:
        setStoryText('Một ngày nọ Phương gặp một bạn nữ sinh đẹp và tài năng...');
        break;
      case 1:
      ...
      case 2:
        setBoyLeft('left-[30%]');
        setStoryText('Sau một thời gian, Phương sử dụng khả năng tìm kiếm của một developer...');
        break;
      case 3:
        setStoryText('Phương đã tìm được thông tin của bạn nữ và biết bạn đang học chuyên ngành Brse Hàn Quốc...');
        break;
      case 4:
      ...
      case 7:
        ...
        setStoryText('Phương nghe xong buồn và quyết tâm sẽ master tiếng Hàn...');
        setBoySrc(sadBoyImage);
        break;
      case 8:
        setStoryText('Phương quyết tâm sẽ master tiếng Hàn và hiểu văn hóa của Hàn Quốc thông qua game của Phương tự dev...');
        setBoyLeft('left-[100%]');
  ...
  return (
    <div className="relative w-[1500px] h-[800px] overflow-hidden bg-pink-300">
      <div className="fixed top-0 left-0 w-full p-4 bg-white/70 backdrop-blur-md flex justify-between items-center">
        <div>{storyText}</div>
        <div >
        <button onClick={handleNext} className="bg-blue-500 text-white p-2 rounded mr-2">Next</button>
        <button onClick={handleSkip} className="bg-blue-500 text-white p-2 rounded">Skip</button>
        </div>
      ...
      <div className={`absolute bottom-[50%] left-[45%] bg-white p-2.5 rounded-lg shadow-lg transition-all duration-2000 ${chatBoxOpacity}`}>Cậu đồng ý làm ny tớ nhé?</div>
      <div className={`absolute bottom-[50%] left-[45%] bg-white p-2.5 rounded-lg shadow-lg transition-all duration-2000 ${chatBox2Opacity}`}>Cậu rất tốt nhưng tiếng Hàn cậu kém...</div>
      <div className={`absolute bottom-[50%] left-[45%] bg-white p-2.5 rounded-lg shadow-lg transition-all duration-2000 ${chatBox3Opacity}`}>Khi nào cậu master tiếng Hàn thì hãy nghĩ đến chuyện này...</div>
      <img src={gameImage} alt='Game' className={`absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 transition-opacity duration-2000 ${gameImageOpacity} w-[80%]`} />
    </div>
  );
}
